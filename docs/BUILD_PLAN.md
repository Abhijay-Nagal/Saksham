...
- **Session 3 (demo-readiness fixes + living Mitthu + brand).** Story: the whole dialogue panel is the tap target, and a tap mid-typewriter now finishes the line (the interval used to keep overwriting it). Card back scrolls (it was `pointer-events:none`; it opts in once flipped, and a tap on it flips back). Play is only height-locked (`md:h-dvh`) during the story, so quiz and results scroll when zoomed; the card overlay centres via an inner `min-h-full` column. Read-aloud is `SpeechControls` (speaker toggles stop, plus pause/resume and restart), backed by `useSpeech()` in `lib/speech.ts`.
- Mitthu is now an SVG (`components/avatar/MitthuBird.tsx`) with CSS-keyframe parts (`mb-*` in index.css): blink with a lid, head tilt, wing stretch, hop, tail sway, and a flapping fly pose. `MitthuArrival` flies him onto a branch on onboarding step 0 (waits for the splash via `lib/splash.ts`). `TownMitthu` perches by the recommended building, visits nearby trees every 13–22s, shows a portal "Call Mitthu back" button while away, flies to the new building after an unlock, and keeps facts up ~60ms/char (4.5–12s). No trips in calm mode.
- Gotcha: a child's `useLayoutEffect` runs before the parent's ref is attached, so TownMitthu measures `anchor.parentElement`, not a map ref. The town map is `overflow-clip` (not hidden) so focus/scrollIntoView can never slide the scene sideways.
- Brand: `components/brand/Logo.tsx` (peacock shield + open book + live Mitthu on the shoulder), used in the top bar and Credits; `public/favicon.svg` matches. `Splash` plays on every load (~2.3s, tap to skip, ~0.9s static in calm) and never over `/help`.
- Also fixed: "Need help?" was missing on Profiles and Onboarding (`FloatingHelp`); Teacher table's sr-only spans widened the page on phones (scroller is now `relative`); top bar crowding at 768–1024px; no AudioContext is created before the first gesture. Verified headless (Playwright + Edge, scratchpad only) at 1366x768, 910x512 (150% zoom) and 390x844 on the dev and production builds: no console errors.
- The two missing sprites are now committed (`pledge` from `Raising hands/Default/3D/raising_hands_3d_default.png`, `zipper` from `Zipper-mouth face/3D/zipper-mouth_face_3d.png`), so the live site has no 404s. Still open for the human: the HANDOFF.md fact check.
- **Session 4: Hindi.** `settings.language` ('en' | 'hi', device-wide, persisted; the persist `merge` fills it in for older saves). UI strings: `content/strings.hi.json`; `t()` falls back to English per key. Content: text-only overlays in `content/hi/` (world, help, community, buildings/*) laid over the English by `localize()` in `lib/content.ts`. Only strings change; the `KEEP` keys (ids, story wiring, answers are numbers, helpline `number`, `law`/`source` citations) always come from English. `world`/`help`/`community`/`allBuildings` are live `let` exports swapped by `setContentLanguage`, and `LanguageRoot` in App remounts the routes on a change (the splash sits outside so it doesn't replay). Read `world`/`community` inside components, never at module scope.
- `LanguageToggle` (names the target language: "हिंदी" / "English") is in the top bar, next to the floating Need help? on Profiles/Onboarding, and on the Help Centre (silent). Me has an English | हिंदी choice. Read-aloud picks a hi-IN voice in Hindi (never an English voice). The typewriter types by grapheme (`Intl.Segmenter`) so Devanagari clusters don't flash half-formed.
- The Hindi was machine-authored (by Claude) and has NOT been reviewed by a fluent speaker. Before a real release, have someone check it, especially `content/hi/help.json` and the card `teen` text. Legal citations (`law`, `source`) deliberately stay in English. The brand name stays "Saksham"; the mascot is "मिट्ठू" in Hindi.
